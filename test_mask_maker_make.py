...
class PhotoViewer:
    def __init__(self,path_denoised, path_mask, filenames_img, filenames_mask):
        ...
        white_btn = tk.Button(self.image_frame, text="Gray", width=10, command=lambda:self.set_color('gray'))
    ...
    def delete_area(self, event):
        if self.mask_cv is None:
            self.mask_cv = cv2.imread(self.mask_path, 0)
        if self.brush_color is not None and self.brush_color != 0:
            self.mask_cv[self.start_y:self.end_y, self.start_x:self.end_x] = self.brush_color
        else:
